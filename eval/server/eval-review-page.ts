  ...
  <div id="inventory-view" style="display:none">
    <div class="content" style="max-width:1100px">
      <section class="panel">
        ...
        <p class="muted">Paste a JSON array of vendor products (e.g. generated by an LLM) and save. The filter, report, and live tabs use the new aliases immediately. <code>id</code> is optional (derived from vendor + product); <code>aliases</code> should include product abbreviations and alternate spellings.</p>
        <div id="inventory-summary" class="badges" style="margin-bottom:8px"></div>
        <textarea id="inventory-json" spellcheck="false" style="min-height:340px;font:12px/1.5 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace"></textarea>
        <div class="actions">
          <span>
...
Return ONLY the JSON array, no commentary.</pre>
  ...
  <script>
    ...
    function switchTab(tab) {
      state.tab = tab;
      for (const name of ['label', 'live', 'report', 'inventory']) {
    ...
    async function loadAll() {
      await Promise.all([loadCandidates(), loadReport(), loadLive(), loadInventory()]);
    }
    ...
    async function loadInventory() {
      ...
      renderInventorySummary(data.vendors);
      document.getElementById('inventory-status').textContent = '';
    }

    function renderInventorySummary(vendors) {
      document.getElementById('inventory-summary').innerHTML = vendors.map((v) =>
        badge(v.vendor + ' / ' + v.product + ' (' + v.aliases.length + ' aliases)',
          v.criticality === 'critical' ? 'bad' : v.criticality === 'high' ? 'warn' : 'good')
      ).join('');
    }
    ...
    async function saveInventory() {
      ...
      document.getElementById('inventory-json').value = JSON.stringify(data.vendors, null, 2);
      renderInventorySummary(data.vendors);
      status.textContent = 'Saved: ' + data.vendors.length + ' products' +
        (data.added.length ? ', added ' + data.added.join(', ') : '') +
        (data.removed.length ? ', removed ' + data.removed.join(', ') : '') +
        '. Report and live tabs now use the new aliases.';
      await loadReport();
    }
