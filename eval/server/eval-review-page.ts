  ...
  <div id="inventory-view" style="display:none">
    <div class="content" style="max-width:1100px">
      ...
      <section class="panel">
        ...
        <textarea id="inventory-add" spellcheck="false" placeholder='[{"vendor": "Okta", "product": "Workforce Identity Cloud", "aliases": ["Okta WIC"], "criticality": "high", "inProduction": true}]' style="min-height:120px;font:12px/1.5 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace"></textarea>
        <div class="actions">
...
Return ONLY the JSON array, no commentary.</pre>
  ...
  <script>
    ...
    async function loadInventory() {
      ...
      state.inventory = data.vendors;
      state.inventoryEditing = null;
    ...
    function renderInventoryTable() {
      ...
      for (const button of container.querySelectorAll('[data-action]')) {
        ...
        button.addEventListener('click', () => {
          if (action === 'edit') { state.inventoryEditing = index; renderInventoryTable(); }
          if (action === 'cancel') { state.inventoryEditing = null; renderInventoryTable(); }
    ...
    async function saveInventoryRow(index) {
      const updated = state.inventory.slice();
    ...
    async function addInventoryFromJson() {
      ...
      const ok = await postInventory(state.inventory.concat(additions), 'Added ' + additions.length + ' product(s).', status);
      if (ok) box.value = '';
    }
